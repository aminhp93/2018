    ...
    componentDidMount() {
                ...
                "operands": [
                    {
                        "operation": {
                            "operator": "or",
                            "operands": [
                                { "expression": { "left": "Recommend.All|1M", "operation": "in_range", "right": [0, 0.5] } },
                                { "expression": { "left": "Recommend.All|1M", "operation": "in_range", "right": [0.5, 1] } }
                            ]
                        }
                    }]
            },
        ...
        axios.post(url, obj)
            .then(response => {
                ...
                if (response.data) {
                    const filtered_data = response.data.data.filter(item => {
                        // console.log(item.d[4])
                        return item && item.d && item.d[4] > 0.04

                    })
                    console.log(filtered_data.length)
                    this.gridApi.setRowData(filtered_data)
